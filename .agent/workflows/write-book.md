---
name: write-book
description: Chạy quy trình có kiểm soát để tạo hoặc sửa sách, giáo trình, lesson, module, chapter hoặc tài liệu đào tạo trong project ikhEdu_lessons.
version: 1.0.0
requires_skills: ikhedu-authoring
artifact_outputs: brief, source-map, architecture, draft, qa-report, handoff
---
# Write Book / Textbook
...
3. Chọn profile đầu ra trước khi viết: `lesson-package` nếu mục tiêu là dạy khái niệm/buổi học; `problem-package` nếu mục tiêu là bài toán chấm tự động. Nếu package thuộc một nhóm có nhiều buổi/bài, đọc `@../skills/ikhedu-authoring/references/group-index-template.md` và lập/cập nhật README/index cấp nhóm. Đọc template profile tương ứng. Lập kiến trúc: mục lục, chapter/module map, prerequisite map và danh sách thuật ngữ. Với học liệu, làm backward design: outcomes -> evidence/assessment -> activities/content.
4. Tạo ma trận alignment. Bảo đảm mỗi learning outcome có assessment/evidence và mỗi phần nội dung có mục đích.
5. Tạo hoặc sửa artifact theo profile đã chọn. Với `lesson-package`, dùng `@../skills/ikhedu-authoring/references/lesson-package-template.md`, tách README/index cấp nhóm, `Ly_Thuyet.md`, `Bai_Tap.md` và code. Với `problem-package`, dùng `@../skills/ikhedu-authoring/references/problem-package-template.md`, tách `De_Bai.md`, `Huong_Dan_Giang_Day.md`, `solution.cpp` và `test/`. Với chapter/module tổng quát, dùng `@../skills/ikhedu-authoring/references/chapter-template.md`. Đánh dấu claim bằng `CLAIM-...`; cập nhật evidence ledger ngay khi dùng source.
...
7. Đọc và hoàn thành `@../skills/ikhedu-authoring/references/qa-checklist.md`. Đối chiếu glossary, cross-reference, bảng/hình, câu hỏi và đáp án. Với lesson package, kiểm tra README -> lesson -> exercise -> code; với problem package, kiểm tra statement -> teacher guide -> solution -> tests.
