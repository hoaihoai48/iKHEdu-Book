...
**Quy chuẩn kiến trúc dữ liệu tối giản (Không nhồi nhét cú pháp):** Trong Phần I và các chương nền tảng đầu (Sắp xếp, Tham lam, Tìm kiếm nhị phân...), **tuyệt đối KHÔNG dùng `struct`, `class`, và KHÔNG dùng `pair`**. Ưu tiên tuyệt đối các bài toán và thao tác trên kiểu dữ liệu nguyên bản (`int`, `long long`, `double`, `char`, `string`, `vector<int>`). Khi bài toán bắt buộc phải lưu và sắp xếp nhiều thuộc tính đi cùng nhau (như mốc thời gian `start`, `finish`), **BẮT BUỘC dùng `vector<vector<int>>` (vector lồng nhau / mảng 2 chiều)** để học sinh tận dụng kiến thức mảng 2 chiều sẵn có và cơ chế so sánh tự động của `sort()`, không tạo gánh nặng cú pháp mới.
