...
- **G0.3 ID/versioning (dinh chinh):** ma problem trong khoa: `sca_lXX_pYY_ten_bai` (chu thuong, giong quy uoc `pya_*` cua khoa Python: doi tien to, giu so + ten). `IKH-xxxx` la ma global duy nhat cua Problem Library (lesson chi placement/tham chieu); gan + map 1–1 khi nhap kho chung. `prob_*`/`SCA-Lxx-Pyy` ban cu chi la nhan noi bo, khong dung lam ma artifact. Schema version `0.1.0`.
