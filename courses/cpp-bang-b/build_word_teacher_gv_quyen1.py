#!/usr/bin/env python3
"""
BUILD SCRIPT: SÁCH GIÁO VIÊN C++ TẬP 1 (BÀI 01 - 07, 108 BÀI TOÁN)
...
7. Header trang: Tab Stop right w:pos="9899", Watermark logo image13.jpeg đầy đủ 3 headers, đường kẻ ngang Straight Connector 1 màu đen 1.5pt.
   Header text: "Giáo trình C++ - Sách giáo viên - Quyển 1"
...
def build_tap1_markdown(manifest_data):
    md_parts = []
    ...
    print(f"  → Tổng hợp Markdown Quyển 1: {len(tap1_lessons)} bài học, {prob_count} bài toán.")
    return "".join(md_parts)

def convert_markdown_to_docx(full_md, out_docx):
    print("🔧 Đang chạy Pandoc Markdown → DOCX...")
    intermediate_md = BASE_DIR / "_build_intermediate_gv_tap1.md"
...
def post_process_teacher_docx(docx_path, manifest_data):
    print("🎨 Đang hậu xử lý Design System chuẩn sách giáo viên Quyển 1...")
    ...
    header_book_text = "Giáo trình C++ - Sách giáo viên - Quyển 1"
    ...
    for idx, para in enumerate(doc.paragraphs):
        ...
        if style_name == "Heading 1" or (para._p.pPr is not None and para._p.pPr.find(qn("w:pStyle")) is not None and para._p.pPr.find(qn("w:pStyle")).attrib.get(qn("w:val")) == "Heading1"):
            ...
            if not is_preface and not is_toc:
                bm_name = f"_toc_bm_t1_{len(toc_headings)}"
    ...
    sync_headers(docx_path, "Giáo trình C++ - Sách giáo viên - Quyển 1")
        ...
        with zipfile.ZipFile(temp_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                ...
                if item.filename == "word/styles.xml":
                    ...
                    # Parse bằng lxml để chuẩn hóa docPr ID
...
def main():
    print("================================================================================")
    print("🚀 BẮT ĐẦU XUẤT BẢN SÁCH GIÁO VIÊN C++ TẬP 1 (BÀI 01 - 07, 108 BÀI TOÁN)")
    print("================================================================================")
    ...
    # 1. Sinh Markdown
    full_md = build_tap1_markdown(manifest_data)
