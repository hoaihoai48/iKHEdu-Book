...
def build_tap3_markdown(manifest_data):
    ...
    tap3_lessons = manifest_data["lessons"][16:24]
    prob_count = 0
...
def post_process_teacher_docx(docx_path, manifest_data):
    ...
    tap3_codes = set(p["code"] for l in manifest_data["lessons"][16:24] for p in l["problems"])
    ...
    # Exact page mapping for Q3 Teacher Guide (319 pages total)
    EXACT_PAGES_GV_Q3 = [
        "2",   # CHƯƠNG 06: QUY HOẠCH ĐỘNG (DYNAMIC PROGRAMMING)
        "2",   # Bài 17: Quy hoạch động 2D & bài toán cái túi (Knapsack)
        "39",  # Bài 18: Quy hoạch động chuỗi: LCS & Edit Distance
        "76",  # CHƯƠNG 07: CẤU TRÚC DỮ LIỆU NÂNG CAO
        "76",  # Bài 19: Cấu trúc dữ liệu STL: Set, map & heap
        "115", # Bài 20: Cấu trúc dữ liệu ngăn xếp (stack) & Monotonic Stack
        "154", # Bài 21: Cấu trúc dữ liệu hàng đợi (queue, deque) & Monotonic Deque
        "193", # CHƯƠNG 08: ĐỒ THỊ & CÂY TRUY VẤN ĐOẠN
        "193", # Bài 22: Lý thuyết đồ thị cơ bản: Duyệt BFS & DFS
        "233", # Bài 23: Đồ thị lưới 2D, kỹ thuật Flood Fill & tính chất cây
        "274"  # Bài 24: Cấu trúc cây phân đoạn (Segment Tree) & Fenwick Tree (BIT)
    ]
    ...
    if p_toc_heading:
        ...
        for item_idx, item in enumerate(toc_headings):
            ...
            bm = item["bm_name"]
            page_str = EXACT_PAGES_GV_Q3[item_idx] if item_idx < len(EXACT_PAGES_GV_Q3) else "1"
