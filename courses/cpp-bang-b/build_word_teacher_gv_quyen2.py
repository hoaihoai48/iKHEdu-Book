#!/usr/bin/env python3
"""
BUILD SCRIPT: SÁCH GIÁO VIÊN C++ TẬP 2 (BÀI 08 - 14, 110 BÀI TOÁN)
...
7. Header trang: Tab Stop right w:pos="9899", Watermark logo image13.jpeg đầy đủ 3 headers, đường kẻ ngang Straight Connector 1 màu đen 1.5pt.
   Header text: "Giáo trình C++ - Sách giáo viên - Quyển 2"
...
def build_tap2_markdown(manifest_data):
    md_parts = []
    
    # Lời nói đầu (Heading 1)
    md_parts.append("# Lời nói đầu\n\n")
    md_parts.append(
        "Cuốn **Giáo trình C++ — Sách giáo viên — Quyển 2** là tập tiếp theo trong bộ tài liệu đào tạo "
        "lập trình thuật toán C++ nâng cao iKHEDU, dành riêng cho quý thầy cô giáo và các huấn luyện viên chuyên trách đội tuyển tin học. "
        ...
        "cho toàn bộ **110 bài toán thực hành** từ Bài 08 đến Bài 14 thuộc các chủ đề thuật toán trọng tâm: Đồng dư thức & lũy thừa nhị phân, "
        "Xử lý số nguyên lớn (BigInt), Thuật toán đệ quy & cây gọi hàm, Kỹ thuật chia để trị, Thuật toán quay lui & nhánh cận, "
        "Quy hoạch động 1D & dãy con tăng dài nhất, và Quy hoạch động 2D & bài toán cái túi (Knapsack).\n\n"
    ...
    print(f"  → Tổng hợp Markdown Quyển 2: {len(tap2_lessons)} bài học, {prob_count} bài toán.")
    return "".join(md_parts)

def convert_markdown_to_docx(full_md, out_docx):
    print("🔧 Đang chạy Pandoc Markdown → DOCX...")
    intermediate_md = BASE_DIR / "_build_intermediate_gv_tap2.md"
...
def post_process_teacher_docx(docx_path, manifest_data):
    print("🎨 Đang hậu xử lý Design System chuẩn sách giáo viên Quyển 2...")
    ...
    for idx, para in enumerate(doc.paragraphs):
        ...
        if style_name == "Heading 1" or (para._p.pPr is not None and para._p.pPr.find(qn("w:pStyle")) is not None and para._p.pPr.find(qn("w:pStyle")).attrib.get(qn("w:val")) == "Heading1"):
            ...
            if not is_preface and not is_toc:
                bm_name = f"_toc_bm_t2_{len(toc_headings)}"
    ...
    sync_headers(docx_path, "Giáo trình C++ - Sách giáo viên - Quyển 2")
...
def main():
    print("================================================================================")
    print("🚀 BẮT ĐẦU XUẤT BẢN SÁCH GIÁO VIÊN C++ TẬP 2 (BÀI 08 - 14, 110 BÀI TOÁN)")
    print("================================================================================")
    ...
    # 1. Sinh Markdown
    full_md = build_tap2_markdown(manifest_data)
