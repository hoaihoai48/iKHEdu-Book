#!/usr/bin/env python3
"""
BUILD SCRIPT: SÁCH GIÁO VIÊN C++ QUYỂN 2 (CHƯƠNG 05 - 07, BÀI 13 - 21, 135 BÀI TOÁN)
Chuẩn khuôn 100% theo python-giaovien-full.docx & cpp-giaovien-quyen-1.docx:
...
7. Header trang: Tab Stop right w:pos="9899", Watermark logo image13.jpeg đầy đủ 3 headers, đồng bộ đường kẻ ngang Straight Connector 1 màu đen 1.5pt.
8. Mục lục: Hyperlink tương tác 3 Chương + 9 Bài học vừa vặn 1 trang, không vỡ trang.
"""
...
TEMPLATE_DOCX = BASE_DIR / "c++-level-1-quyen-2.docx"
OUT_DOCX = BASE_DIR / "cpp-giaovien-quyen-2.docx"
DIAGRAMS_MAP_FILE = BASE_DIR / "problem_diagrams_q2_map.json"
...
def build_q2_markdown(manifest_data):
    ...
    md_parts.append(
        "Cuốn **Giáo trình C++ — Sách giáo viên — Quyển 2** là tập tiếp theo hoàn thiện bộ giáo trình đào tạo "
        "lập trình thuật toán thi đấu nâng cao iKHEDU, dành riêng cho quý thầy cô giáo và các huấn luyện viên bồi dưỡng học sinh giỏi. "
        "Tài liệu này tập trung vào 3 trụ cột thuật toán trọng yếu bậc nhất trong các kỳ thi Tin học trẻ Bảng B và HSG THCS/THPT: "
        "**Quy hoạch động chuyên sâu** (DP 1D, Knapsack 2D, String DP/LCS/Edit Distance), "
        "**Cấu trúc dữ liệu nâng cao** (STL Set, Map, Heap, Monotonic Stack & Deque), và "
        "**Lý thuyết đồ thị & Cây truy vấn đoạn** (Duyệt BFS/DFS, Lưới 2D Flood Fill, Segment Tree & Fenwick Tree).\n\n"
        "Toàn bộ **135 bài toán thực hành** từ Bài 13 đến Bài 21 đều được trang bị hệ thống **hướng dẫn giải chi tiết (Editorial)** chuẩn mực: "
        "phân tích bản chất và chiến lược thuật toán, công thức chuyển trạng thái toán học, bảng mô phỏng chạy tay từng bước (Dry Run Table) "
        "trên chính số liệu thật của Sample 1, phân tích bẫy lỗi kinh điển và mã nguồn tham khảo tối ưu (Fast I/O, Safe Input, 0 std::).\n\n"
    )

    current_chapter = None
    q2_lessons = [l for l in manifest_data["lessons"] if l.get("chapter", 0) in [5, 6, 7]]
    prob_count = 0
    ...
    print(f"  → Tổng hợp Markdown Quyển 2: {len(q2_lessons)} bài học, {prob_count} bài toán.")
    return "".join(md_parts)

def convert_markdown_to_docx(full_md, out_docx):
    print("🔧 Đang chạy Pandoc Markdown → DOCX...")
    intermediate_md = BASE_DIR / "_build_intermediate_gv_q2.md"
...
def post_process_teacher_docx(docx_path, manifest_data):
    ...
    header_book_text = "Giáo trình C++ - Sách giáo viên - quyển 2"
        ...
        with zipfile.ZipFile(temp_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                ...
                if item.filename == "word/styles.xml":
                    ...
                    # Gán ID duy nhất từ 1000 cho mọi docPr hình ảnh trong document.xml tránh trùng ID 1 của footer3
                    curr_id = 1000
...
def sync_headers_quyen2(docx_path):
    print("📏 Đang đồng bộ Header và Đường kẻ ngang Straight Connector 1 cho 100% các trang...")
    temp_zip = docx_path.parent / f"_temp_sync_hdr_{docx_path.name}"
    
    # Lấy mẫu header3 chuẩn từ c++-level-1-quyen-1 hoặc cpp-giaovien-quyen-1.docx
    ...
    # Loại bỏ tab thừa (child 4) trong mẫu cũ, chỉ giữ đúng 1 tab căn phải chuẩn pos="9899"
...
def main():
    print("================================================================================")
    print("🚀 BẮT ĐẦU XUẤT BẢN SÁCH GIÁO VIÊN C++ QUYỂN 2 (FULL KHUÔN PYTHON)")
    print("================================================================================")
    ...
    # 1. Sinh Markdown
    full_md = build_q2_markdown(manifest_data)
