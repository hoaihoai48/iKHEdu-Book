...
#### Câu 1 (Độ phức tạp):
Thuật toán Lũy thừa nhị phân (Binary Exponentiation) tính $A^B \pmod M$ có độ phức tạp thời gian là:
* A. $\mathcal{O}(B)$
* B. $\mathcal{O}(\sqrt{B})$
...
> *Giải thích:* Phép chia trên vành modulo bắt buộc phải nhân với nghịch đảo của mẫu số: $A \times B^{-1} \pmod M$.

---

#### Câu 5 (Điều kiện tồn tại Nghịch đảo):
Nghịch đảo modulo của số nguyên $A$ theo modulo $M$ (tức số $X$ sao cho $A \times X \equiv 1 \pmod M$) **chắc chắn tồn tại** khi và chỉ khi:
...
Để trả lời $10^5$ truy vấn tính số tổ hợp $C(N, K) \pmod{10^9 + 7}$ với $N, K \le 10^6$ trong tổng thời gian dưới $0.1\text{s}$, phương pháp tối ưu nhất là gì?
...
> *Giải thích:* Tiền xử lý $\mathcal{O}(N)$ cho phép tính $C(N, K) = fact[N] \times invFact[K] \times invFact[N-K] \pmod M$ trong đúng $\mathcal{O}(1)$ phép nhân.
...
> *Giải thích:* Vì $\frac{1}{(i-1)!} = \frac{1}{i!} \times i$, do đó `invFact[i - 1] = (invFact[i] * i) % MOD`. Ta chỉ cần tính `invFact[N] = power(fact[N], MOD - 2)` rồi đi lùi về $0$.
...
* A. $B \gets B \pmod M$
* B. **(Đáp án đúng)** $B \gets B \pmod{(M - 1)}$
* C. $B \gets B \pmod{(M + 1)}$
* D. $B \gets B \pmod{\sqrt{M}}$
> *Giải thích:* Vì $M$ là số nguyên tố và $\gcd(A, M) = 1$, theo Fermat nhỏ $A^{M - 1} \equiv 1 \pmod M$. Do đó $A^B = A^{q(M-1) + r} = (A^{M-1})^q \times A^r \equiv 1^q \times A^r = A^r \pmod M$ với $r = B \pmod{M - 1}$.
...
Thuật toán Euclid mở rộng tìm cặp nghiệm nguyên $(x, y)$ cho phương trình $Ax + My = \gcd(A, M)$. Nếu $\gcd(A, M) = 1$, giá trị $x \pmod M$ đại diện cho đại lượng nào?
