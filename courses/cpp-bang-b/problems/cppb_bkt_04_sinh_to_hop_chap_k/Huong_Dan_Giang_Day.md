...
* **Kỹ năng cốt lõi:** Nắm vững và làm chủ kỹ thuật: Sinh Tất Cả Tổ Hợp Chập K Của N.
...
* **Dữ liệu đầu vào:** - Một dòng duy nhất chứa 2 số nguyên $N, K$ ($1 \le K \le N \le 16$).
* **Yêu cầu cốt lõi:** Cho 2 số nguyên $N, K$ ($1 \le K \le N \le 16$). Hãy sinh tất cả các tổ hợp chập $K$ của $\{1, 2, \dots, N\}$ theo thứ tự từ điển bằng thuật toán Quay Lui.
...
3 4
```
* **Phân tích thực thi:** Có đúng C(4, 2) = 6 tổ hợp chập 2 của {1, 2, 3, 4}.

---

## 6. Phân Tích Độ Phức Tạp Thời Gian & Không Gian
* **Thời gian thực thi (Time Complexity):** $\Theta(K \cdot C_N^K)$
* **Bộ nhớ ngăn xếp (Call Stack Space):** $\Theta(K)$ (Độ sâu tối đa: $K$)
* **Bộ nhớ phụ trợ (Auxiliary Memory):** $\Theta(K)$
* **Ghi chú phân tích:** Duyệt start_val tăng dần để sinh các tổ hợp không lặp.
