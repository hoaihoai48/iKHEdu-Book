...
* **Kỹ năng cốt lõi:** Nắm vững và làm chủ kỹ thuật: Đổi Tiền Xu Ít Nhất (B&B Coin Change).
...
* **Dữ liệu đầu vào:** - Dòng 1: Hai số nguyên $N, S$ ($1 \le N \le 15, 1 \le S \le 100$).
- Dòng 2: $N$ số nguyên dương $C_1, \dots, C_N$ ($1 \le C_i \le 100$).
* **Yêu cầu cốt lõi:** Cho $N$ mệnh giá tiền xu $C_1, C_2, \dots, C_N$ (số lượng mỗi loại không giới hạn) và số tiền cần đổi $S$. Hãy tìm số lượng đồng xu ít nhất để đổi đúng số tiền $S$ bằng thuật toán Nhánh Cận (Branch and Bound). Nếu không đổi được, in `-1`.
...
3
```
* **Phân tích thực thi:** 11 = 5 + 5 + 1 (dùng đúng 3 đồng xu).

---

## 6. Phân Tích Độ Phức Tạp Thời Gian & Không Gian
* **Thời gian thực thi (Time Complexity):** Exponential worst-case (Phụ thuộc chất lượng Bound)
* **Bộ nhớ ngăn xếp (Call Stack Space):** $\mathcal{O}(S / C_{\min})$ (Độ sâu tối đa: $\lfloor S / C_{\min} \rfloor$ với giả thiết $C_i \ge 1$)
* **Bộ nhớ phụ trợ (Auxiliary Memory):** $\Theta(N)$
* **Ghi chú phân tích:** Lower Bound: count + (remain + c[0] - 1)/c[0] >= best_coins -> Prune.
