...
- Quy trình gồm hai bước với hai biến `a` và `b` trong lời giải: dùng `các khối hỏi và đợi cho từng biến` để cắt dòng `8 5` thành `8` và `5` rồi cất vào `a` và `b`, sau đó in ba dòng `nói (a + b)`, `nói (a - b)`, `nói (a * b)` cho ra `13`, `3`, `40`.
...
|------|-----------|--------------|------------------|
| 1 | `a, b = các khối hỏi và đợi cho từng biến` với bàn phím gõ `8 5` | `a = 8`, `b = 5` | (chưa in gì) |
...
- Bẫy 3: quên `int()` khi cắt dòng, viết `a, b = câu trả lời` rồi `nói (a + b)` thì với mẫu `8 5` máy nối chữ thành `85` thay vì `13`. Cách sửa: bọc `map(int, ...)` để đổi thành số.
