# Hướng Dẫn Giảng Dạy: Số lớn nhất trong hai số
...
- Bẫy 1 — chỉ đọc một số: bạn nhỏ viết `a = int(câu trả lời)` rồi `nói (a)`. Với mẫu `-15` và `8`, chương trình chỉ in `-15`, thiếu hẳn số thứ hai. Cách sửa: đọc thêm dòng `b = int(câu trả lời)` rồi mới so sánh.
