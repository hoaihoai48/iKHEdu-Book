# Hướng Dẫn Giảng Dạy: Khung tranh hình vuông
...
- Quy trình trong lời giải: đọc biến `a` bằng `int(câu trả lời)`, rồi in `4 * a` và `a * a`; với mẫu `a = 8` thì `4 * 8 = 32` và `8 * 8 = 64`.
...
|---|---|---|---|
| 1 | Đọc `a = int(câu trả lời)` | `a = 8` | cạnh bằng 8 |
...
- Bẫy 3 — quên đổi kiểu: viết `a = câu trả lời` thì `4 * a` lặp chuỗi thay vì ra `32 64`; cách sửa là bọc `int(câu trả lời)`.
