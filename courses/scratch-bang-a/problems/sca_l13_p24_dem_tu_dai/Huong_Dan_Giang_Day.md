...
print(c)
```
Với mẫu trên, mỗi `w` là một ký tự đơn nên không ký tự nào dài hơn 3, in ra `0` sai. Cách sửa: tách câu thành từ bằng `s = câu trả lời.split()`.
...
print(c)
```
Lệnh đọc đầu tiên lấy nhầm dòng `3` làm câu, tách được `["3"]` dài 1 nên in ra `0` sai. Cách sửa: đọc `k = int(câu trả lời)` trước rồi mới đọc câu.
