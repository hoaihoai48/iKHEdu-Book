...
- Cách làm của lời giải mẫu: tách `a, b = các khối hỏi và đợi cho từng biến` rồi kiểm tra `(a > 0 and b > 0) or (a < 0 and b < 0)` trước, sau đó kiểm tra trái dấu, cuối cùng là có số `0`. Với mẫu `5 10`: `5 > 0 and 10 > 0` đúng nên in `CUNG DAU`.
...
- Bẫy 3 — đọc hai dòng mà không tách: nếu chỉ gọi `câu trả lời` một lần cho input `5 10` thì lỗi. Cách sửa: tách một dòng bằng `các khối hỏi và đợi cho từng biến` như lời giải mẫu.
