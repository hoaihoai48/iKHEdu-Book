...
- `(7 mod 2) = 1` nên cờ `la_snt` giữ nguyên `True` và in ra `YES`.
- Thầy cô nhắc thêm hai mốc trong đề: `n = 1` in `NO` (nhỏ hơn `2`), `n = 9` in `NO` (vì `(9 mod 3) == 0`).
...
| `i = 2` | `(7 mod 2) = 1` | không chia hết, `la_snt` vẫn `True` |
...
```
với `n = 1` vòng lặp rỗng nên vẫn in `YES`, là kết quả sai. Sửa lại: giữ nhánh `if n < 2: nói ("NO")` như bài giải.
