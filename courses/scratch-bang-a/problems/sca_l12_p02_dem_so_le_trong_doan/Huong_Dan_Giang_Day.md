...
- Với `A = 3`, `B = 8`: từ `1` tới `8` có `(8 + 1) // 2 = 4` số lẻ (`1, 3, 5, 7`); từ `1` tới `2` (phần trước `A`) có `làm tròn xuống của (3 / 2) = 1` số lẻ (`1`).
...
| --- | --- | --- |
| `1..8` | `(8 + 1) // 2 = 4` | `1, 3, 5, 7` |
| `1..2` (trước `A = 3`) | `làm tròn xuống của (3 / 2) = 1` | `1` |
...
- Bẫy 1: lấy `(b - a) // 2` làm đáp án. Với mẫu `3 8` được `(8 - 3) // 2 = 2`, thiếu mất một số lẻ. Sửa lại: `(b + 1) // 2 - làm tròn xuống của (a / 2)`.
...
- Bẫy 3: nhầm `làm tròn xuống của (a / 2)` thành `(a + 1) // 2`. Với mẫu `3 8` được `4 - 2 = 2`, là kết quả sai. Sửa lại: phần trừ đi là `làm tròn xuống của (a / 2)` (số lẻ nhỏ hơn `A`).
...
> - nói (b + 1 chia nguyên 2 - a chia nguyên 2)
