...
- Quy trình gồm ba bước với hai biến `a` và `b` trong lời giải: đọc `12` vào `a` và `5` vào `b` bằng `int(hỏi và đợi.strip())`, rồi tính `a * b` tức `12 * 5 = 60` và in ra.
...
|------|-----------|--------------|------------------|
| 1 | `a = int(hỏi và đợi.strip())` với dòng 1 gõ `12` | `a = 12` | (chưa in gì) |
...
- Bẫy 2: quên `int()`, viết `a = hỏi và đợi.strip()` rồi `nói (a * b)` thì máy lặp chữ, cho ra kết quả lạ thay vì `60`. Cách sửa: viết `a = int(hỏi và đợi.strip())` và `b = int(hỏi và đợi.strip())`.
- Bẫy 3: đọc hai số trên một dòng bằng `map(int, hỏi và đợi.split())` trong khi đề cho hai dòng riêng thì chương trình sẽ chờ thiếu số. Cách sửa: đọc hai lần `int(hỏi và đợi.strip())` cho đúng hai dòng.
