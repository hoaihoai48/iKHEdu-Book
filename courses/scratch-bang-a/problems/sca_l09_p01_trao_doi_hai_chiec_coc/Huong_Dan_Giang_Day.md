...
- Quy trình từng bước với đúng tên biến trong lời giải:
  - Bước 1: `a, b = map(int, câu trả lời.split())` đọc một dòng, tách thành hai số nguyên. Với mẫu `5 12` thì `a = 5`, `b = 12`.
...
|---|---|---|---|---|
| 1 | `a, b = map(int, câu trả lời.split())` với dòng `5 12` | 5 | 12 | (chưa in gì) |
| 2 | `nói (b, a)` | 5 | 12 | `12 5` |
...
print(a, b)
```
- Bẫy 2: quên tách dòng nhập bằng `split()`, viết `a = int(câu trả lời)` rồi đọc tiếp dòng thứ hai không tồn tại. Với mẫu chỉ có một dòng `5 12`, cách viết sai này đọc `a = 5` rồi chờ thêm dữ liệu. Cách sửa: dùng `map(int, câu trả lời.split())` để đọc cả hai số trên cùng một dòng.
```text
a = int(câu trả lời)
b = int(câu trả lời)
print(b, a)
