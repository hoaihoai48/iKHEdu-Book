...
- Quy trình từng bước với đúng tên biến trong lời giải:
  - Bước 1: `n = câu trả lời` đọc số. Với mẫu, `n = 2035`.
  - Bước 2: đặt hai giỏ `chan = 0` và `le = 0`.
  - Bước 3: lặp `while n > 0`, mỗi lần lấy `d = (n mod 10)`; nếu `(d mod 2) == 0` thì `chan` tăng 1, ngược lại `le` tăng 1; rồi gọt `n = làm tròn xuống của (n / 10)`.
...
| Lần lặp | `n` trước | `d = (n mod 10)` | `(d mod 2) == 0`? | `chan` sau | `le` sau | `n` sau |
|---|---|---|---|---|---|---|
...
```
- Bẫy 3: quên gọt `n` trong vòng lặp, `n` mãi bằng 2035 nên lặp vô tận. Cách sửa: cuối mỗi lần lặp phải `n = làm tròn xuống của (n / 10)`.
```text
n = câu trả lời
chan = 0
le = 0
