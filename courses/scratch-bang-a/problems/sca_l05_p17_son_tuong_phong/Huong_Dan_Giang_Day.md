...
- Bẫy 2 — quên nhân đơn giá: viết `nói ((a * h) - (x * y))` thì với mẫu ra `16` thay vì `800000`; cách sửa là nhân thêm `g`.
- Bẫy 3 — đọc năm dòng riêng: dùng năm lần `câu trả lời` thì với mẫu một dòng sẽ bị treo chờ; cách sửa là tách một dòng bằng `split()`.
