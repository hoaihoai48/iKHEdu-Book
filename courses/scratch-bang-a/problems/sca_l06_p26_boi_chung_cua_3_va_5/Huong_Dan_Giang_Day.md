...
- Bản chất của bài này là số `n` phải chia hết cho cả `3` và `5` cùng lúc, tức `(n mod 3) == 0` và `(n mod 5) == 0`.
- Cách làm của lời giải mẫu: đọc `n`, nếu cả hai điều kiện đúng thì in `YES`, ngược lại in `NO`. Với mẫu `n = 15`: `(15 mod 3) = 0` đúng và `(15 mod 5) = 0` đúng nên in `YES`.
...
|---|---|---|---|
| 1 | Đọc input | `n = 15` | — |
| 2 | Kiểm tra `(15 mod 3) == 0`? Đúng | tiếp tục | — |
| 3 | Kiểm tra `(15 mod 5) == 0`? Đúng, cả hai đúng | rẽ nhánh `if` | — |
| 4 | In kết quả | — | `YES` |

---

## 3. Lưu ý & Bẫy lỗi thường gặp
- Bẫy 1 — dùng `or` thay vì `and`: bạn nhỏ viết `if (n mod 3) == 0 or (n mod 5) == 0`. Với `n = 9` sẽ in `YES`, sai vì `9` không chia hết cho `5`. Cách sửa: nối hai điều kiện bằng `and`.
