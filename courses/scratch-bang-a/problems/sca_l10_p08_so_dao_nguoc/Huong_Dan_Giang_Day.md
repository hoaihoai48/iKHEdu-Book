...
- Quy trình từng bước với đúng tên biến trong lời giải:
  - Bước 1: `n = câu trả lời` đọc số. Với mẫu, `n = 1234`.
  - Bước 2: đặt `dao = 0` làm số đảo đang xây.
  - Bước 3: lặp `while n > 0`, mỗi lần lấy `(n mod 10)` đắp vào `dao`, rồi gọt `n = làm tròn xuống của (n / 10)`.
...
| Lần lặp | `n` trước | Chữ số `(n mod 10)` | `dao` trước | `dao` sau (`dao * 10 + chữ số`) | `n` sau |
|---|---|---|---|---|---|
...
- Bẫy 1: quên nhân `dao` với 10, viết `dao = dao + (n mod 10)`. Với mẫu sẽ cộng `4 + 3 + 2 + 1 = 10`, là kết quả sai. Cách sửa: viết đủ `dao = dao * 10 + (n mod 10)`.
```text
n = câu trả lời
dao = 0
while n > 0:
...
```
- Bẫy 2: quên gọt `n` trong vòng lặp, `n` mãi bằng 1234 nên lặp vô tận. Cách sửa: cuối mỗi lần lặp phải `n = làm tròn xuống của (n / 10)`.
```text
n = câu trả lời
dao = 0
while n > 0:
