...
- Cách làm của lời giải mẫu: đọc `d = float(câu trả lời)` rồi rẽ nhánh từ cao xuống thấp. Với mẫu `d = 8.5`: `8.5 >= 8.0` đúng nên in `GIOI`.
...
- Bẫy 1 — đọc `int` thay vì `float`: bạn nhỏ viết `d = câu trả lời`. Với mẫu `8.5` chương trình sẽ lỗi. Cách sửa: đọc `float` như lời giải mẫu.
