...
- Cách làm của lời giải mẫu: đọc `den = câu trả lời.upper()` rồi rẽ ba nhánh, mỗi nhánh còn nhận thêm cách viết chữ đầy đủ (`DO`, `VANG`, `XANH`). Với mẫu `do`: viết hoa thành `DO`, rơi vào nhánh một nên in `DUNG LAI`.
...
- Bẫy 1 — quên viết hoa: bạn nhỏ viết `den = câu trả lời` rồi so với `'D'`. Với mẫu `do` chữ thường sẽ không khớp nhánh nào, không in gì. Cách sửa: thêm `.upper()` như lời giải mẫu.
