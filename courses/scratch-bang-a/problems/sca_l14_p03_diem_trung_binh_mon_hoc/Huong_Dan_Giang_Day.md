...
print(f"{tb:.2f}")
```
Với mẫu toàn điểm nguyên vẫn ra `8.50`, nhưng điểm `7.5` trong đề là số thực nên `int("7.5")` gây lỗi chương trình. Cách sửa: đọc bằng `map(float, câu trả lời.split())`.
