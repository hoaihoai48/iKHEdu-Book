...
- Quy trình gồm hai bước với biến `ten` trong lời giải: đọc chuỗi `"Nam"` vào `ten` bằng `câu trả lời` (có gọt khoảng trắng thừa ở hai đầu), rồi dùng phép cộng chuỗi `"Xin chao ban " + ten + "!"` để nối ba mảnh lại thành câu hoàn chỉnh.
...
|------|-----------|--------------|------------------|
| 1 | `ten = câu trả lời` với bàn phím gõ `Nam` | `ten = "Nam"` | (chưa in gì) |
...
- Bẫy 1: đổi tên sang số bằng `ten = int(hỏi và đợi)` thì với mẫu `Nam` chương trình báo lỗi vì `Nam` không phải số. Cách sửa: tên là chữ nên chỉ viết `ten = câu trả lời`.
