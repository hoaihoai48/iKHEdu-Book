...
- Quy trình gồm ba bước với hai biến `s1` và `s2` trong lời giải: đọc nguyên văn hai dòng `"25"` vào `s1` và `"30"` vào `s2` bằng `câu trả lời` (giữ dạng chữ), dòng 1 in `s1 + s2` tức `"25" + "30" = "2530"`, dòng 2 in `int(s1) + int(s2)` tức `25 + 30 = 55`.
...
|------|-----------|--------------|------------------|
| 1 | `s1 = câu trả lời` với dòng 1 gõ `25` | `s1 = "25"` | (chưa in gì) |
| 2 | `s2 = câu trả lời` với dòng 2 gõ `30` | `s2 = "30"` | (chưa in gì) |
...
- Bẫy 1: đổi sang số ngay từ đầu bằng `s1 = int(hỏi và đợi)` thì dòng ghép `s1 + s2` với mẫu `25` và `30` sẽ tính `25 + 30 = 55` ở cả hai dòng, mất dòng `2530`. Cách sửa: giữ nguyên chữ bằng `câu trả lời`, chỉ đổi sang số ở dòng cộng.
