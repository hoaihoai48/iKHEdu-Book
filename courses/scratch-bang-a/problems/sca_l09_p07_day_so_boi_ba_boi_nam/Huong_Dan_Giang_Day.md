# Hướng Dẫn Giảng Dạy: Dãy số bội ba bội năm
...
- Quy trình từng bước với đúng tên biến trong lời giải:
  - Bước 1: `n = int(câu trả lời.strip())` đọc số lượng cần lấy. Với mẫu, `n = 6`.
  - Bước 2: đặt `res = []` đựng kết quả và `num = 1` là số đang xét.
  - Bước 3: lặp `while len(res) < n`, mỗi lần kiểm tra `if num % 3 == 0 or num % 5 == 0` thì thêm `str(num)` vào `res`, rồi tăng `num` thêm 1.
  - Bước 4: in `" ".join(res)`.
...
| `num` | `num % 3 == 0` | `num % 5 == 0` | Có thêm vào `res` không | `res` sau bước này |
|---|---|---|---|---|
| 1 | sai | sai | không | [] |
| 2 | sai | sai | không | [] |
| 3 | đúng | sai | có | [`3`] |
