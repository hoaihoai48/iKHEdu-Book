...
- Quy trình gồm hai bước với ba biến `d`, `m`, `y` trong lời giải: dùng `map(int, hỏi và đợi.split())` để cắt dòng `4 9 2026` thành `4`, `9`, `2026` rồi cất vào `d`, `m`, `y`, sau đó `nói (d, m, y, sep="/")` đặt dấu `/` vào giữa các số và in ra `4/9/2026`.
...
|------|-----------|--------------|------------------|
| 1 | `d, m, y = map(int, hỏi và đợi.split())` với bàn phím gõ `4 9 2026` | `d = 4`, `m = 9`, `y = 2026` | (chưa in gì) |
...
- Bẫy 3: đọc bằng ba lệnh `int(hỏi và đợi)` riêng trong khi đề cho cả ba số trên một dòng thì chương trình sẽ chờ thiếu số sau khi đã gõ `4 9 2026`. Cách sửa: đọc một dòng rồi cắt bằng `map(int, hỏi và đợi.split())`.
...
> - nói (kết hợp d và " " và m và " " và y và " " và sep="/")
