...
|---|---|---|---|
| 1 | Đọc `p = int(câu trả lời)` | `p = 30` | chu vi 30 |
| 2 | Đọc `a = int(câu trả lời)` | `a = 5` | cạnh biết 5 |
...
- Bẫy 1 — đọc hai số một dòng: viết `p, a = map(int, câu trả lời.split())` thì với mẫu mỗi số một dòng sẽ bị lỗi thiếu số; cách sửa là đọc hai lần `câu trả lời` riêng.
- Bẫy 2 — quên chia đôi chu vi: viết `nói (p - a)` thì với mẫu ra `25` thay vì `10`; cách sửa là `p // 2 - a`.
