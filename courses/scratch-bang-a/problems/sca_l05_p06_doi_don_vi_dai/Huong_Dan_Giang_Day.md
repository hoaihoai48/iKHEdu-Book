...
- Quy trình trong lời giải: đọc biến `m` bằng `câu trả lời`, rồi in `m * 100` và `m * 1000`; với mẫu `m = 3` thì `3 * 100 = 300` và `3 * 1000 = 3000`.
...
- Bẫy 2 — đọc hai số: viết `m, n = các khối hỏi và đợi cho từng biến` thì với mẫu chỉ có một số `3` sẽ bị lỗi thiếu số; cách sửa là chỉ đọc một số `m`.
...
> - nói (kết hợp m * 100 và ' ' và m * 1000)
