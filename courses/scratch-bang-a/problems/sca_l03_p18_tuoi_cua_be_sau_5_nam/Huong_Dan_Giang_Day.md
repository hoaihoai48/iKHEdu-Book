...
- Quy trình gồm hai bước với biến `n` trong lời giải: đọc `8` vào `n` bằng `câu trả lời`, rồi tính `n + 5` tức `8 + 5 = 13` và in ra.
...
|------|-----------|--------------|------------------|
| 1 | `n = câu trả lời` với bàn phím gõ `8` | `n = 8` | (chưa in gì) |
...
- Bẫy 3: quên `int()`, viết `n = câu trả lời` rồi `nói (n + 5)` thì chương trình báo lỗi vì không cộng chữ với số được. Cách sửa: viết `n = câu trả lời`.
