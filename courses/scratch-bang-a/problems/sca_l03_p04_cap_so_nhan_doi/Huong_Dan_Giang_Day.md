...
- Quy trình gồm hai bước với biến `a` trong lời giải: đọc `15` vào `a` bằng `câu trả lời` (có gọt khoảng trắng thừa quanh số), rồi tính `a * 2` tức `30` và in ra.
...
|------|-----------|--------------|------------------|
| 1 | `a = câu trả lời` với bàn phím gõ `15` | `a = 15` | (chưa in gì) |
...
- Bẫy 1: quên `int()`, viết `a = câu trả lời` rồi `nói (a * 2)` thì với mẫu `15` máy lặp chữ hai lần, hiện `1515` thay vì `30`. Cách sửa: viết `a = câu trả lời`.
