# Hướng Dẫn Giảng Dạy: Đếm số lượng số chẵn trong mảng
...
- Quy trình trong lời giải với các biến `n`, `a`, `x`:
  ...
  - Với mỗi `x`, kiểm tra `x % 2 == 0`: `2` đạt, `5` trượt, `8` đạt, `10` đạt, `13` trượt.
...
| 3 | Xét `2` | `2 % 2 == 0` nên đếm 1 |
| 4 | Xét `5` | `5 % 2 == 1` nên bỏ qua |
| 5 | Xét `8` | đếm 2 |
| 6 | Xét `10` | đếm 3 |
| 7 | Xét `13` | bỏ qua |
...
```
Với mẫu trên in ra `2` (số `5` và `13`), không khớp đáp án mẫu `3`. Cách sửa: điều kiện đúng là `x % 2 == 0`.
- Bẫy 2 — quên so sánh, chỉ viết `if x % 2`:
```python
n = int(input().strip())
a = list(map(int, input().split()))
print(sum(1 for x in a if x % 2))
```
Với mẫu trên, `x % 2` khác 0 với số lẻ nên lại đếm số lẻ, in ra `2` sai. Cách sửa: viết rõ `if x % 2 == 0`.
...
```
Với mẫu trên in ra `[2, 8, 10]`, không khớp đáp án mẫu `3`. Cách sửa: đếm bằng `sum(1 for x in a if x % 2 == 0)`.
