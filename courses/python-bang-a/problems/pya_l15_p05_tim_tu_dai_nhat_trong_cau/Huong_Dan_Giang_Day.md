...
|---|---|---|---|
| 1 | `s = input()` | `s = "Hoc lap trinh rat thu vi"` | 6 từ |
| 2 | `words = s.split()` | 6 từ, dài nhất là `trinh` (5 chữ) | `max` theo `len` |
| 3 | `print(longest)` | màn hình hiện `trinh` | khớp Output mẫu |
...
- Bẫy 2: in độ dài thay vì từ `print(len(longest))`. Đoạn sai:
```python
s = input()
words = s.split()
...
print(len(longest))
```
Với mẫu trên in ra `5`, đáp án đúng là `trinh`. Cách sửa: in trực tiếp `print(longest)`.
