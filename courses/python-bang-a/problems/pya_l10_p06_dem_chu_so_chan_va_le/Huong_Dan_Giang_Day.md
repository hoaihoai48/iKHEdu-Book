...
- Quy trình từng bước với đúng tên biến trong lời giải:
  - Bước 1: `n = int(input())` đọc số. Với mẫu, `n = 2035`.
  ...
  - Bước 4: in `print(chan, le)` (giỏ chẵn trước, giỏ lẻ sau).
...
print(chan, le)
```
- Bẫy 2: in ngược thứ tự `print(le, chan)`. Với mẫu tuy hai giỏ bằng nhau đều là 2 nên trùng, nhưng với số như 246 (chẵn 2, lẻ 1) sẽ in `1 2`, là kết quả sai. Cách sửa: in `print(chan, le)`.
```python
n = int(input())
chan = 0
le = 0
