...
* **Chuẩn code chuẩn:** Đọc 2 số bằng `input()`, ép kiểu `int`, chỉ in đúng 1 số, không in thừa.
...
1. **In thừa giải thích:** Viết `print("Dien tich:", ans)` thay vì chỉ `print(ans)` gây `kết quả sai`.
2. **Quên ép kiểu:** Dùng `a = input()` rồi tính `a * h` tạo chuỗi lặp thay vì số học.
3. **Dùng `/` thay `//`:** In ra `12.0` thay vì `12`, chương trình kiểm tra so khớp chuỗi sẽ trừ điểm.
