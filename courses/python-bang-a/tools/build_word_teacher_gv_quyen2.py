#!/usr/bin/env python3
"""
BUILD SCRIPT: SÁCH GIÁO VIÊN PYTHON — QUYỂN 2 (BÀI 07 - 14, CHƯƠNG 3, 4 & 5, 130 BÀI TOÁN)
...
7. Header 3 mặt: Watermark logo alt="logo_in" ở trung tâm, đường kẻ ngang Straight Connector 1, Header text: "Giáo trình Python - Sách giáo viên - Quyển 2".
...
PREFACE_FILE = PYTHON_DIR / "reference" / "Loi_Noi_Dau_GV_Quyen2.md"
OUT_DOCX = PYTHON_DIR / "python-giaovien-quyen-2.docx"
...
def build_q2_markdown(manifest_data):
    """Assemble Markdown for Teacher Book Volume 2 (Lessons 7-14, Chapters 3-5)."""
    md_parts = []
    ...
    q2_lessons = manifest_data["lessons"][6:14]
    prob_count = 0
    ...
    print(f"  → Đã tổng hợp Markdown Quyển 2: {len(q2_lessons)} bài học, {prob_count} bài toán.")
...
def convert_markdown_to_docx(full_md, out_docx):
    ...
    intermediate_md = PYTHON_DIR / f"_build_intermediate_gv_q2.md"
...
def post_process_teacher_docx(docx_path, manifest_data):
    """Áp dụng toàn bộ quy chuẩn Design System cho Sách Giáo Viên Quyển 2."""
    ...
    props.title = "Giáo trình Python — Sách giáo viên — Quyển 2"
    ...
    for idx, para in enumerate(doc.paragraphs):
        ...
        if style_name == "Heading 1" or (para._p.pPr is not None and para._p.pPr.find(qn("w:pStyle")) is not None and para._p.pPr.find(qn("w:pStyle")).attrib.get(qn("w:val")) == "Heading1"):
            ...
            if not is_preface and not is_toc:
                bm_name = f"_toc_bm_q2_{len(toc_headings)}"
        ...
        # Heading 3 (Tên bài toán)
            ...
            bm_name = f"_toc_bm_q2_p_{len(toc_headings)}"
            bm_start = parse_xml(f'<w:bookmarkStart {nsdecls("w")} w:id="{len(toc_headings)}" w:name="{bm_name}"/>')
            bm_end = parse_xml(f'<w:bookmarkEnd {nsdecls("w")} w:id="{len(toc_headings)}"/>')
            para._p.insert(0, bm_start)
            para._p.append(bm_end)
    ...
    print(f"🖼️ Đang chèn hình ảnh minh họa bối cảnh cho Quyển 2...")
    q2_codes = set(p["code"] for l in manifest_data["lessons"][6:14] for p in l["problems"])
    for prob_code in q2_codes:
    ...
    EXACT_PAGES_GV_Q2 = [
        "2",   # CHƯƠNG 03: BÀI TOÁN SỐ HỌC & TÁCH CHỮ SỐ
        "2",   # Bài 07: Quy luật dãy số và tam giác số
        "27",  # Bài 08: Kỹ thuật tách chữ số và xử lý số nguyên qua vòng lặp while
        "56",  # Bài 09: Ước số, Bội số và Số nguyên tố
        "77",  # Bài 10: Đếm số theo quy luật và số đặc biệt
        "94",  # CHƯƠNG 04: DANH SÁCH (LIST) & THỐNG KÊ
        "94",  # Bài 11: Danh sách và thao tác cơ bản
        "140", # Bài 12: Thống kê danh sách và sắp xếp
        "164", # CHƯƠNG 05: XỬ LÝ CHUỖI KÝ TỰ
        "164", # Bài 13: Chuỗi ký tự — Chỉ số, cắt lát và duyệt ký tự
        "179"  # Bài 14: Duyệt chuỗi, biến đổi ký tự và tách từ
    ]
    ...
    if p_toc_heading:
        ...
        for item_idx, item in enumerate(toc_headings):
            ...
            if item.get("is_chapter") or item.get("is_lesson"):
                page_str = EXACT_PAGES_GV_Q2[chap_lesson_count] if chap_lesson_count < len(EXACT_PAGES_GV_Q2) else "1"
                chap_lesson_count += 1
            else:
                page_str = "1"
    ...
    sync_headers(docx_path, "Giáo trình Python - Sách giáo viên - Quyển 2")
...
def main():
    print("================================================================================")
    print("🚀 BẮT ĐẦU XUẤT BẢN SÁCH GIÁO VIÊN PYTHON — QUYỂN 2 (CHƯƠNG 3, 4 & 5, BÀI 07 - 14)")
    print("================================================================================")

    with open(MANIFEST_FILE, "r", encoding="utf-8") as f:
        manifest_data = json.load(f)

    # 1. Sinh Markdown hoàn chỉnh cho Quyển 2
    ...
    print("\n================================================================================")
    print(f"🎉 HOÀN THÀNH XUẤT BẢN SÁCH GIÁO VIÊN QUYỂN 2: {OUT_DOCX.name}")
